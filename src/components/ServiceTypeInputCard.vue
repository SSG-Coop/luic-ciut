    ...
    <v-row no-gutters class="mt-3">
      <!-- <v-col cols="12">
        <v-text-field
          ...
          label="Quantity"
          variant="outlined"
          :rules="requiredNumberRule('Quantity')"
        ...
        <v-tooltip location="top">
          <template v-slot:activator="{ props }">
            <v-text-field
              ...
              v-bind="props"
              :label="`${$t('forms.fields.capitalCostIntensity')} (${units})`"
              variant="outlined"
              :rules="
                requiredNumberRule($t('forms.fields.capitalCostIntensity'))
              "
              @update:modelValue="updateCapCost"
            ></v-text-field>
          </template>
          <span>{{ $t('forms.servicesEnterZero') }}</span>
        </v-tooltip>
      </v-col>
      <v-col cols="12">
        <v-text-field
      ...
      <v-col cols="12">
        <v-text-field
          ...
          :label="`${$t('forms.fields.maintCostIntensity')} (${units})`"
          variant="outlined"
      ...
      <v-col cols="12">
        <v-text-field
          v-model="maintCost"
          :label="`${$t('forms.fields.opAndMaintCost')} ($/${$t('general.yearAbbr')})`"
          variant="outlined"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const capCostIntensity = ref(
      servicesConfig().serviceTypes[props.serviceType].capCostIntensity
    );
    const capCost = ref(
      servicesConfig().serviceTypes[props.serviceType].capCostIntensity *
        props.qty
    ...
    const maintCostIntensity = ref(
      servicesConfig().serviceTypes[props.serviceType].maintCostIntensity
    );
    const maintCost = ref(
      servicesConfig().serviceTypes[props.serviceType].maintCostIntensity *
        props.qty
    );
    const units = ref(servicesConfig().serviceTypes[props.serviceType].units);
